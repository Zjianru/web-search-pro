...
export async function buildDoctorReport(options = {}) {
  ...
  return {
    ...
    renderLane: {
      ...
      runtimeAvailable: renderProvider?.runtimeAvailable ?? false,
      browserFamily: renderProvider?.runtime?.browserFamily ?? null,
      browserBinary: renderProvider?.runtime?.browserPath ?? null,
      ...
      executeJavaScript: config.render.enabled && renderProvider?.runtimeAvailable === true,
...
export function formatDoctorMarkdown(report) {
  ...
  lines.push("");
  lines.push("## Browser Render Lane");
  lines.push("");
  lines.push(`- Enabled: ${report.renderLane.enabled ? "yes" : "no"}`);
  lines.push(`- Policy: ${report.renderLane.policy}`);
  lines.push(`- Runtime available: ${report.renderLane.runtimeAvailable ? "yes" : "no"}`);
  lines.push(`- Wait until: ${report.renderLane.waitUntil}`);
  lines.push(`- Budget: ${report.renderLane.budgetMs}ms`);
  lines.push(`- Blocked resource types: ${report.renderLane.blockTypes.join(", ") || "none"}`);
  lines.push(`- Same-origin only: ${report.renderLane.sameOriginOnly ? "yes" : "no"}`);
